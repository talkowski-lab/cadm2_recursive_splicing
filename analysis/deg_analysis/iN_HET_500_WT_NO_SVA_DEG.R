...
comparison <- c("wt_wt_wt","het_500bp_del")
sampleTable <- sampleTable %>% 
  filter(celltype=="iN") %>%
  filter(genotype %in% comparison)

#sampleTable$colour <- c("het","het","het","het","CADM2_wt","CADM2_wt","CADM2_wt","CADM2_wt","PWS_wt","PWS_wt","PWS_wt","PWS_wt")
...
sampleTable$genotype <- factor(sampleTable$genotype,levels=c("wt_wt_wt","het_500bp_del"))
#sampleTable$colour <- factor(sampleTable$colour, levels=c("CADM2_wt", "PWS_wt","het"))
...
pathList <- pathList[-grep(c("inv|hom_del|.lsf|100kb_het_del"),pathList)]
...
####TPM
gene.length <- 1115.441
x <- counts(ddsHTSeq) / gene.length
tpm.mat <- t( t(x) * 1e6 / colSums(x) )
tpm.mat[rownames(tpm.mat)=="ENSG00000175161",]
...
pdf("..\\..\\output\\uncorrected_iN_WT_vs_het_del_500.pdf")
cell_pca
dev.off() 
...
#m=model.matrix(~genotype, data=colData(ddssva))
#m=as.data.frame(m)
#colnames(m)
#m1=m[,3:ncol(m)]
#X=as.matrix(m1)
...
#cleanY=2^cleany
write.table(y, "..\\..\\output\\iN_WT_vs_het_del_500_no_pws_no_sva_cleany.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)

corrected_PCA_genotype <- rnaPCA(cleany,sampleTable = colData(dds), "colour", PC1 = 1,PC2 = 2, textLabel = "Samples") 
pdf("..\\..\\output\\corrected_iN_WT_vs_het_del_500_PWS_labels.pdf")
...
library(corrplot)
source("C:\\Users\\ricar\\Downloads\\fd_mouse_tissue\\mouse\\Scripts\\rnaPCA.R")
test_pc <- rnaPCA(log2(counts(dds, normalized=T) + 1),sampleTable = colData(dds), "genotype", PC1 = 1,PC2 = 2, returnData = T)
...
pdf("..\\..\\output\\corplot_uncorrected_cleaned_genotype_model_iN_het_del_500_vs_wt.pdf")
...
pdf("..\\..\\output\\corplot_corrected_cleaned_genotype_model_iN_het_del_500_vs_wt_pws.pdf")
corrplot(cormat$r,type="upper", order = "original", p.mat = cormat$P, sig.level = 0.05, insig = "blank")
dev.off()
...
write.table(as.data.frame(rownames(diff_genotype)), "..\\..\\output\\iN_DEG_WT_vs_het_del_500_q_0.1_ensembl_no_sva.txt", row.names=FALSE, quote=FALSE, col.names = FALSE)                                                                                                                                
write.table(res_genotype, "..\\..\\output\\iN_DEG_stats_WT_vs_het_del_500_no_sva.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)
write.table(as.data.frame(rownames(dds)),"..\\..\\output\\iN_background_ensembl_WT_vs_het_del_500_no_sva.txt",row.names=FALSE, quote=FALSE, col.names = FALSE)
...
pdf("..\\..\\output\\iN_WT_vs_het_del_500_no_sva__qqplot.pdf")
...
pdf("..\\..\\output\\iN_WT_vs_het_del_500_no_sva_volcano.pdf")
EnhancedVolcano(res,
                lab = gene_ids,
                x = 'log2FoldChange',
                y = 'padj',
