...
#sampleTable$colour <- c("hom","hom","hom","hom","hom","hom","hom","hom","CADM2_wt","CADM2_wt","CADM2_wt","CADM2_wt","PWS_wt","PWS_wt","PWS_wt","PWS_wt")
...
#sampleTable$colour <- factor(sampleTable$colour, levels=c("CADM2_wt", "PWS_wt","hom"))
...
pdf("..\\..\\output\\uncorrected_iN_WT_vs_hom_del_no_sva.pdf")
cell_pca
dev.off() 
...
#cleanY=2^cleany
write.table(y, "..\\..\\output\\iN_WT_vs_hom_del_cleany_no_sva.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)

corrected_PCA_genotype <- rnaPCA(y,sampleTable = colData(dds), "genotype", PC1 = 1,PC2 = 2, textLabel = "Samples") 
pdf("..\\..\\output\\corrected_iN_WT_vs_hom_del_no_sva_label.pdf")
...
pdf("..\\..\\output\\corplot_uncorrected_cleaned_genotype_model_iN_hom_del_vs_wt_no_sva.pdf")
...
pdf("..\\..\\output\\corplot_corrected_cleaned_genotype_model_iN_hom_del_vs_wt_pws.pdf")
corrplot(cormat$r,type="upper", order = "original", p.mat = cormat$P, sig.level = 0.05, insig = "blank")
dev.off()
...
write.table(as.data.frame(rownames(diff_genotype)), "..\\..\\output\\iN_DEG_WT_vs_hom_del_no_sva_q_0.1_ensembl.txt", row.names=FALSE, quote=FALSE, col.names = FALSE)                                                                                                                                
write.table(res_genotype, "..\\..\\output\\iN_DEG_stats_WT_vs_hom_del_no_sva.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)
write.table(as.data.frame(rownames(dds)),"..\\..\\output\\iN_background_ensembl_WT_vs_hom_del_no_sva.txt",row.names=FALSE, quote=FALSE, col.names = FALSE)
...
pdf("..\\..\\output\\iN_WT_vs_hom_del_no_sva_qqplot.pdf")
...
pdf("..\\..\\output\\iN_WT_vs_hom_del_no_sva_volcano.pdf")
EnhancedVolcano(res,
                lab = gene_ids,
                x = 'log2FoldChange',
                y = 'padj',
