...
comparison <- c("wt_wt_wt","het_100kb_del")
sampleTable <- sampleTable %>% 
  filter(celltype=="iN") %>%
  filter(genotype %in% comparison)
#sampleTable$colour <- c("CADM2_wt","het","het","CADM2_wt","CADM2_wt","het","CADM2_wt","het","PWS_wt","PWS_wt","PWS_wt","PWS_wt")
...
sampleTable$genotype <- factor(sampleTable$genotype,levels=c("wt_wt_wt","het_100kb_del"))
...
pathList <- pathList[-grep(c("inv|hom_del|.lsf|500bp_het_del"),pathList)]
...
pdf("..\\..\\output\\uncorrected_iN_WT_vs_het_del_100_no_sva.pdf")
cell_pca
dev.off() 
...
#cleanY=2^cleany
write.table(y, "..\\..\\output\\iN_WT_vs_het_del_100_no_pws_no_sva_cleany.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)

corrected_PCA_genotype <- rnaPCA(cleany,sampleTable = colData(dds), "colour", PC1 = 1,PC2 = 2, textLabel = "Samples")
pdf("..\\..\\output\\corrected_iN_WT_vs_het_del_100_PWS_labels.pdf")
...
pdf("..\\..\\output\\corplot_uncorrected_cleaned_genotype_model_iN_het_del_100_vs_wt_no_sva.pdf")
...
pdf("..\\..\\output\\corplot_corrected_cleaned_genotype_model_iN_het_del_100_vs_wt_pws.pdf")
corrplot(cormat$r,type="upper", order = "original", p.mat = cormat$P, sig.level = 0.05, insig = "blank")
dev.off()
...
write.table(as.data.frame(rownames(diff_genotype)), "..\\..\\output\\iN_DEG_WT_vs_het_del_100_no_sva_q_0.1_ensembl.txt", row.names=FALSE, quote=FALSE, col.names = FALSE)                                                                                                                                
write.table(res_genotype, "..\\..\\output\\iN_DEG_stats_WT_vs_het_del_100_no_sva.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)
write.table(as.data.frame(rownames(dds)),"..\\..\\output\\iN_background_ensembl_WT_vs_het_del_100_no_sva.txt",row.names=FALSE, quote=FALSE, col.names = FALSE)
...
pdf("..\\..\\output\\iN_WT_vs_het_del_100_no_sva_qqplot.pdf")
...
pdf("..\\..\\output\\iN_WT_vs_het_del_100_no_sva_volcano.pdf")
EnhancedVolcano(res,
                lab = gene_ids,
                x = 'log2FoldChange',
                y = 'padj',
