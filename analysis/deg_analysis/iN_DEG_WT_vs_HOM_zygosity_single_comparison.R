...
sampleTable$zygosity <- factor(sampleTable$zygosity,levels=c("wt","hom"))
...
sampleTable$genotype <- factor(sampleTable$genotype,levels=c("wt_wt_wt","hom_500bp_del"))
...
pathList <- pathList[-grep(c("100kb_het_del|inv|.lsf|500bp_het_del|r1B6"),pathList)]
...
pdf("..\\..\\output\\uncorrected_iN_WT_vs_hom_no_sva_no_pws_single.pdf")
cell_pca
dev.off() 
...
#cleanY=2^cleany
write.table(y, "..\\..\\output\\iN_WT_vs_hom_no_sva_no_pws_single.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)

corrected_PCA_genotype <- rnaPCA(y,sampleTable = colData(dds), "genotype", PC1 = 1,PC2 = 2, textLabel = "Samples") 
pdf("..\\..\\output\\corrected_iN_WT_vs_hom_no_sva_no_pws_single_labels.pdf")
...
test1$replicate <- as.numeric(as.factor(test1$replicate))
test1$zygosity <- as.numeric(as.factor(test1$zygosity))
test1$mutation_type<-as.numeric(as.factor(test1$mutation_type))
test1$genotype <- as.numeric(as.factor(test1$genotype))
cormat<- rcorr(as.matrix(test1))
diag(cormat$P) <- 0

pdf("..\\..\\output\\corplot_uncorrected_cleaned_genotype_model_iN_WT_vs_hom_no_sva_no_pws_single.pdf")
...
library(Hmisc)
library(corrplot)
test_pc <- rnaPCA(log2(counts(ddssva, normalized=T) + 1),sampleTable = colData(ddssva), "genotype", PC1 = 1,PC2 = 2, returnData = T)
test1 <- cbind(test_pc[,c("genotype","mutation_type","zygosity", "replicate","librarySize",
                          "sizeFactor")],test_pc[1:10])
...
pdf("..\\..\\output\\corplot_corrected_cleaned_genotype_model_iN_hom_vs_wt_no_sva.pdf")
corrplot(cormat$r,type="upper", order = "original", p.mat = cormat$P, sig.level = 0.05, insig = "blank")
dev.off()
...
write.table(as.data.frame(rownames(diff_genotype)), "..\\..\\output\\iN_WT_vs_hom_no_sva_no_pws_single_q_0.1_ensembl.txt", row.names=FALSE, quote=FALSE, col.names = FALSE)                                                                                                                                
write.table(res_genotype, "..\\..\\output\\iN_DEG_stats_WT_vs_hom_single_no_sva_no_pws.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)
write.table(as.data.frame(rownames(dds)),"..\\..\\output\\iN_background_ensembl_WT_vs_hom_single_no_sva_no_pws.txt",row.names=FALSE, quote=FALSE, col.names = FALSE)
...
pdf("..\\..\\output\\iN_WT_vs_hom_no_sva_qqplot_single.pdf")
...
pdf("..\\..\\output\\iN_WT_vs_hom_no_sva_volcano_single.pdf")
EnhancedVolcano(res,
                lab = gene_ids,
                x = 'log2FoldChange',
                y = 'padj',
