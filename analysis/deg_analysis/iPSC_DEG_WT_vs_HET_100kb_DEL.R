...
pathList <- pathList[-grep("hom|inv|500bp",pathList)]
...
wt <- grep("wt",gc_names)
het <- grep("100kb_het_del",gc_names)
...
pdf("..\\..\\output\\uncorrected_iPSC_WT_vs_HET_100kb_DEL.pdf")
cell_pca
dev.off() 
...
cleanY=2^cleany
write.table(cleany, "..\\..\\output\\iPSC_WT_vs_HET_100kb_DEL_cleany.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)
...
pdf("..\\..\\output\\corplot_uncorrected_cleaned_genotype_model_het_100kb_del_vs_wt.pdf")
...
write.table(as.data.frame(rownames(diff_genotype)), "..\\..\\output\\iPSC_DEG_WT_vs_het_100kb_del_q_0.1_ensembl_names.txt", row.names=FALSE, quote=FALSE, col.names = FALSE)                                                                                                                                
write.table(res_genotype, "..\\..\\output\\iPSC_DEG_stats_WT_vs_HET_100kb_DEL_500bp_del.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)
write.table(as.data.frame(rownames(dds)),"..\\..\\output\\iPSC_background_ensembl_WT_vs_HET_100kb_DEL_names.txt",row.names=FALSE, quote=FALSE, col.names = FALSE)
...
pdf("..\\..\\output\\iPSC_WT_vs_HET_100kb_del_single_comparison_qqplot.pdf")
...
pdf("..\\..\\output\\iPSC_WT_vs_HET_100kb_DEL_del_single_comparison_volcano.pdf")
EnhancedVolcano(res,
                lab = gene_ids,
                x = 'log2FoldChange',
                y = 'padj',
