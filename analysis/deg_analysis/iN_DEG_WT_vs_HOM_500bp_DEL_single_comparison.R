...
pathList <- pathList[-grep(c("100kb_het_del|inv|.lsf|500bp_het_del"),pathList)]
...
pdf("..\\..\\output\\uncorrected_iN_WT_vs_hom_del_no_sva_no_pws.pdf")
cell_pca
dev.off() 
...
m=model.matrix(~genotype + SV1 + SV2 + SV3 + SV4, data=colData(ddssva))
m=as.data.frame(m)
colnames(m)
m1=m[,3:ncol(m)]
X=as.matrix(m1)
...
cleanY=2^cleany
write.table(y, "..\\..\\output\\iN_WT_vs_hom_del_no_pws_no_sva_cleany.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)
