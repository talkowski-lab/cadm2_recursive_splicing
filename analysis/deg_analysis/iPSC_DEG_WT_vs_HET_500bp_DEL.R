...
pathList <- pathList[-grep("hom|inv|100kb",pathList)]
...
wt <- grep("wt",gc_names)
het <- grep("500bp_het_del",gc_names)
exp=apply(cpm,1,FUN=countsFilter,conditionList=list(wt,het))
dds=ddsHTSeq[exp,]
...
pdf("..\\..\\output\\uncorrected_iPSC_WT_vs_HET_500bp_DEL.pdf")
cell_pca
dev.off() 
...
cleanY=2^cleany
write.table(cleany, "..\\..\\output\\iPSC_WT_vs_HET_500bp_DEL_cleany.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)

corrected_PCA_genotype <- rnaPCA(cleany,sampleTable = colData(dds), "genotype", PC1 = 1,PC2 = 2, textLabel = "Samples") 
pdf("..\\..\\output\\corrected_iPSC_WT_vs_HET_500bp_DEL_labels.pdf")
...
test1 <- cbind(test_pc[,c("genotype","mutation_type","zygosity","librarySize","RIN",
                          "Conc_to_Use",
                          "Input_amount",
                          "Average_Size_bp",
                          ...
                          "Conc_To_use",
                          "sizeFactor")],test_pc[1:8],test_pc[33:34])
...
pdf("..\\..\\output\\corplot_uncorrected_cleaned_genotype_model_het_500bp_del_vs_wt.pdf")
...
pdf("..\\..\\output\\corplot_corrected_cleaned_genotype_model_het_500bp_del_vs_wt.pdf")
corrplot(cormat$r,type="upper", order = "original", p.mat = cormat$P, sig.level = 0.05, insig = "blank")
dev.off()
...
resultsNames(ddssva)
res <- results(ddssva,contrast=c(0,1,0,0))
...
write.table(as.data.frame(rownames(diff_genotype)), "..\\..\\output\\iPSC_DEG_WT_vs_het_500bp_del_q_0.1_ensembl_names.txt", row.names=FALSE, quote=FALSE, col.names = FALSE)                                                                                                                                
write.table(res_genotype, "..\\..\\output\\iPSC_DEG_stats_WT_vs_HET_500bp_del.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)
write.table(as.data.frame(rownames(dds)),"..\\..\\output\\iPSC_background_ensembl_WT_vs_HET_500bp_names.txt",row.names=FALSE, quote=FALSE, col.names = FALSE)
...
pdf("..\\..\\output\\iPSC_WT_vs_HET_500_bp_del_single_comparison_qqplot.pdf")
...
pdf("..\\..\\output\\iPSC_WT_vs_HET_500_bp_del_single_comparison_volcano.pdf")
EnhancedVolcano(res,
                lab = gene_ids,
                x = 'log2FoldChange',
                y = 'padj',
