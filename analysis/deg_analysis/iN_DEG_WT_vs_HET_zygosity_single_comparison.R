...
sampleTable$genotype <- factor(sampleTable$genotype,levels=c("wt_wt_wt","het_500bp_del","het_500bp_inv","het_100kb_del"))
...
pathList <- pathList[-grep(c("hom|.lsf|r1B6|total"),pathList)]
...
pdf("..\\..\\output\\uncorrected_iN_WT_vs_het_no_pws_single.pdf")
cell_pca
dev.off() 
...
cleanY=2^cleany
write.table(y, "..\\..\\output\\iN_WT_vs_het_no_pws_single.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)

corrected_PCA_genotype <- rnaPCA(y,sampleTable = colData(dds), "genotype", PC1 = 1,PC2 = 2, textLabel = "Samples") 
pdf("..\\..\\output\\corrected_iN_WT_vs_het_no_sva_no_pws_single_genotype_labels.pdf")
...
pdf("..\\..\\output\\corplot_corrected_cleaned_genotype_model_iN_het_vs_wt_no_sva_single.pdf")
...
resultsNames(ddssva)
res <- results(ddssva,contrast=c(0,1,0,0,0,0,0))
...
write.table(as.data.frame(rownames(diff_genotype)), "..\\..\\output\\iN_WT_vs_het_sva_no_pws_single_q_0.1_ensembl.txt", row.names=FALSE, quote=FALSE, col.names = FALSE)                                                                                                                                
write.table(res_genotype, "..\\..\\output\\iN_DEG_stats_WT_vs_het_single_sva_no_pws.txt", row.names=TRUE, quote=FALSE, col.names = TRUE)
write.table(as.data.frame(rownames(dds)),"..\\..\\output\\iN_background_ensembl_WT_vs_het_single_sva_no_pws.txt",row.names=FALSE, quote=FALSE, col.names = FALSE)
...
pdf("..\\..\\output\\iN_WT_vs_het_inv_sva_single_qqplot.pdf")
...
pdf("..\\..\\output\\iN_WT_vs_het_inv_sva_volcano_single.pdf")
EnhancedVolcano(res,
                lab = gene_ids,
                x = 'log2FoldChange',
                y = 'padj',
